...
export default function Profile() {
  ...
  return (
    <div id="profile-page">
      {userLoading && <h2 className="loading-data">Loading user data...</h2>}
      {userData?.user && (
        <>
          <Card id="info">
            <ul className="display-user">
              <li>
                <span className="display-user__label">Fullname:</span>{" "}
                <span>
                  {userData?.user.firstName} {userData?.user.lastName}
                </span>
              </li>
              <li>
                <span className="display-user__label">Email:</span>{" "}
          ...
          {userData?.user.pets && userData?.user.pets.length > 0 && (
            <div className="pet-section">
              <h2>
                {userData?.user.firstName} {userData?.user.lastName}'s pets
              </h2>
                ...
                {userData?.user.pets.map((pet) => (
                  <li className="pet-card" key={pet._id}>
                    ...
                    <div className="right-column">
                      ...
                      <p>
                        Birthday: {dayjs(pet.birthdate).format("MMMM D, YYYY")}
                      </p>
                      <p>
                        Age: {currentDateTime.diff(pet.birthdate, "year")}{" "}
                        year(s) old.
                      </p>
                    </div>
                  </li>
